      ...
      <ul className="space-y-2.5">
        {block.items.map((it, i) => (
          <li key={i} className="flex items-start gap-3 text-charcoal/75">
            <span className={`mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full text-[11px] font-bold ${block.ordered ? "bg-violet text-white" : "bg-violet/10 text-violet"}`}>
  ...
  if (block.type === "image") {
    return (
      <figure className="my-2">
        ...
        {block.caption && <figcaption className="mt-2 text-center text-xs text-charcoal/50">{block.caption}</figcaption>}
      </figure>
    );
  }
  if (block.type === "quote") {
    return (
      <blockquote className="rounded-r-xl border-l-4 border-violet bg-violet/[0.05] px-5 py-4">
        <p className="text-lg italic leading-relaxed text-charcoal/80">&ldquo;{block.text}&rdquo;</p>
        {block.cite && <footer className="mt-2 text-sm not-italic text-charcoal/45">{block.cite}</footer>}
      </blockquote>
    );
  }
    ...
    return (
      <div className="flex items-start gap-3 rounded-2xl border border-violet/20 bg-violet/[0.05] p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-relaxed text-charcoal/70">{block.text}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 gap-4 rounded-2xl border border-charcoal/[0.07] bg-charcoal-50/50 p-5 sm:grid-cols-4">
        {block.items.map((s) => (
          <div key={s.label} className="text-center">
            ...
            <p className="mt-2 text-xs leading-snug text-charcoal/55">{s.label}</p>
          </div>
        ))}
      </div>
    );
      ...
      <table className="w-full min-w-[480px] text-left text-sm">
        ...
        <tbody className="divide-y divide-charcoal/[0.07]">
          {block.rows.map((row, i) => (
            <tr key={i}>{row.map((c, j) => <td key={j} className="px-4 py-3 align-top text-charcoal/70">{c}</td>)}</tr>
          ))}
        </tbody>
      </table>
    </div>
      ...
      <article>
        ...
        <header className="relative overflow-hidden bg-charcoal pt-[108px] pb-12 text-white">
          ...
          <div className="container-x relative max-w-3xl">
            ...
            <p className="mt-3 max-w-2xl text-base leading-relaxed text-white/65">{post.excerpt}</p>
          ...
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-14">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-14 border-t border-charcoal/10 pt-10">
                ...
                <p className="mt-1 text-sm text-charcoal/55">Your email address will not be published. Required fields are marked *</p>
